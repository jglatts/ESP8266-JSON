...
void serverOnEveryThing() {
    ...
    server.on("/AllDistances.txt", sendAllDistances);
...
void updatePingSensor() {
    ...
    distance= (duration/2) / 74;    // get distance, in inches
...
/* Display all distances recorded via JSON */
void sendAllDistances() {
    String all_dists_xml;
    // test at 10 KiloBytes
    StaticJsonDocument<10000> all_dists_json;

    all_dists_json["dis_one"] = String(all_dists[0]);
    all_dists_json["dis_two"] = String(all_dists[1]);
    all_dists_json["dis_three"] = String(all_dists[2]);
    all_dists_json["dis_four"] = String(all_dists[3]);
    all_dists_json["dis_five"] = String(all_dists[4]);
    all_dists_json["dis_six"] = String(all_dists[5]);

    serializeJson(all_dists_json, all_dists_xml);
    server.send(200, "text/html", all_dists_xml);
}
...
/*
 * RFID Module
 *  - check a new user in and update settings
 */
void checkUserIn() {
    ++number_of_checkins;
